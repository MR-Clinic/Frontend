...
function Index() {
  ...
  return (
    <>
      <Head>
        <title>Patient Page - Dashboard</title>
        <meta
          property="og:title"
          content="Patient - Dashboard Page"
          key="title"
        />
      </Head>
      {/* modal */}
                ...
                <div className="grid grid-cols-1 my-4 capitalize">
                  <div className="grid grid-cols-2 font-bold gap-2 items-center">
                    ...
                    <div className="relative  before:content-[':'] before:mr-3 py-2 before:font-bold">
                      <span className="normal-case border-2 border-green-900 rounded py-2 px-3 w-8/12">
                        {" "}
                        {datePh}{" "}
                        <BsCalendar2Event className="inline-block ml-3" />
                      </span>
                      <input
                        className=" opacity-0 left-3 absolute"
                        min={today}
                        type="date"
                    ...
                    <p className="font-bold before:content-[':'] before:mr-3">
                      {" "}
                      {cap}
                    </p>
                  </div>
                        ...
                        if (date === "") {
                          ...
                          dispatch(allStore.createVisit(doctor_uid, date))
                            ...
                            .catch((e) => {
                              if (e.status === 500) {
                                swal(
                                  "Anda Sudah Ada Janji Kunjungan",
                                  "Batalkan Janji Kunjungan Untuk Menambahkan Janji Baru",
                                  "error"
                                );
                                setTimeout(() => {
                                  swal.close();
                ...
                {dataAppointment.length !== 0 ? (
                  <>
                    <div className="grid grid-cols-3 justify-between w-full">
                      ...
                      <p className="col-span-2 before:content-[':'] before:mr-2">
                        {dataAppointment[0].doctorName}
                      </p>
                      <p className="font-bold"> Hari Kunjungan </p>
                      <p className="col-span-2 before:content-[':'] before:mr-2">
                        {dataAppointment[0].date}
                      </p>
                      <p className="font-bold"> Alamat </p>
                      <p className="col-span-2 before:content-[':'] before:mr-2">
                        {dataAppointment[0].doctorAddress}
                      </p>
                    </div>

                    <button
                      className="w-3/12 border-red-500 border-2 col-span-1 mt-5 px-2 py-3 rounded-[5px] text-red-700 capitalize text-sm font-bold flex justify-center items-center"
                      onClick={() => cancelJK(dataAppointment[0].visit_uid)}
                    >
                      Batalkan Janji
                      {loadCK ? (
                        <ReactLoading
                  ...
                  <p className="text-xl text-center w-full">
                    Anda Belum Ada Janji Kunjungan
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="pb-5  grid grid-cols-3 max-h-[74vh] max-w-[60vw] overflow-y-scroll z-0">
              {dataDoctor
                ? dataDoctor.map((el, i) =>
                    el.name !== "" ? (
                      <div
                        className="bg-[#324B50] min-h-[150px] pt-3 mt-5 rounded-lg mr-4  drop-shadow-md"
                        key={i}
                      >
                        <div className="flex flex-col justify-between items-center bg-white h-full text-[10px] rounded-b-lg py-3 px-2">
                          ...
                          <div className="flex px-3">
                            <button
                              className="bg-[#324B50] px-3 py-2 rounded-[5px] text-green-100 capitalize text-md font-bold"
                              onClick={(e) => {
                                handleModal(el);
                              }}
                            >
                ...
                ? dataHistory.map((el, i) =>
                    el.status === "completed" ? (
                      <tr className="text-center border-b-2 ">
                        ...
                        <td className="w-3/12">{el.doctorAddress}</td>
